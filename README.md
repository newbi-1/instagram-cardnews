...
- Cloud Secrets의 `ADMIN_EMAIL` 은 판매자 수신함만 (예: `seller@example.com`). OTP는 임의 주소로 보내지 않습니다.
- 로컬 테스트: `EMAIL_DEV_MODE=1` 이면 OTP를 UI에 표시 (운영 Cloud에서는 금지)
...
3. App settings → Secrets 에 `ADMIN_GATE` + `ADMIN_EMAIL` + `RESEND_API_KEY` + 구매자 `[buyers.*]` (+ 선택 `IG_*` / `ADMIN_PASSWORD`). 예: `.streamlit/secrets.toml.example`
